/*
Henter utstyr basert på type (1=lagsbil, 2=terreng, 3=drone)
*/
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!type) {
      return res.status(400).json({ error: 'Mangler type-parameter' });
    }
    ...
    const [rows] = await connection.execute(
      'SELECT p_key, s_description FROM d_utstyr WHERE r_type=? AND b_active=1',
      [type]
    );
